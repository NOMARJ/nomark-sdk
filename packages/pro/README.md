...
Each compute backend handles all 20 compute verbs across data, flow, resilience, trust, and notification categories. Compositions with flow verbs (AWAIT, BRANCH, SPLIT, MERGE, GATE, SIGNAL, TERMINATE) emit a graph-aware dispatcher; non-flow compositions emit the linear-chain dispatcher (byte-exact unchanged). Each surface backend handles 5 of the 11 surface verbs (MONITOR, ARRANGE, DISPLAY, STATUS, GUIDE) — DECIDE/CONFIGURE/EXPLORE/AUTHOR/ONBOARD/COLLECT pending the surface-outcome wave. A resolver strips verbs outside its family with a `VERBS_STRIPPED` count warning — passing a mixed composition is safe.
