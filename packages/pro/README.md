...
Each compute backend handles 13 of the 20 compute verbs (FETCH, VALIDATE, MAP, FILTER, REDUCE, PERSIST, EMIT, ENRICH, DELETE, STREAM, RETRY, COMPENSATE, ERROR); the remaining 7 (flow-category — AWAIT, BRANCH, SPLIT, MERGE, GATE, SIGNAL, TERMINATE) surface as `VERB_UNHANDLED` warnings pending the dispatcher-refactor wave. Each surface backend handles 5 of the 11 surface verbs (MONITOR, ARRANGE, DISPLAY, STATUS, GUIDE). A resolver also strips verbs outside its family with a `VERBS_STRIPPED` count warning (compute resolvers strip surface verbs; surface resolvers strip compute verbs) — passing a mixed composition is safe.
