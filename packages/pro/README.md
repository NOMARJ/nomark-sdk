...
Each compute backend handles all 20 compute verbs across data, flow, resilience, trust, and notification categories. Compositions with flow verbs (AWAIT, BRANCH, SPLIT, MERGE, GATE, SIGNAL, TERMINATE) emit a graph-aware dispatcher; non-flow compositions emit the linear-chain dispatcher (byte-exact unchanged). Each surface backend handles all 11 surface verbs across outcome, present, and respond categories. Compositions with interactive verbs (DECIDE, CONFIGURE, EXPLORE, AUTHOR, ONBOARD, COLLECT) emit an interactive Dashboard signature — `Dashboard(state, dispatch)` — with the host wiring dispatch to its state management; read-only compositions keep the original `Dashboard(state)` signature byte-exact. A resolver strips verbs outside its family with a `VERBS_STRIPPED` count warning — passing a mixed composition is safe.
